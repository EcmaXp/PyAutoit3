...
def clip_get(buf_size: int = DEFAULT_CLIP_GET_BUFFER_SIZE, *, _clip=None) -> str:
    clip = new_buffer_raw(buf_size) if _clip is None else _clip
    API.AU3_ClipGet(clip, buf_size)
    return buffer_as_text(clip)


def clip_put(clip: str) -> None:
    API.AU3_ClipPut(clip)
...
def control_command(title: str, text: str, control: str, command: str, extra: str,
                    buf_size: int = DEFAULT_CONTROL_COMMAND_BUFFER_SIZE, *, _result=None) -> str:
    result = new_buffer(buf_size) if _result is None else _result
    API.AU3_ControlCommand(title, text, control, command, extra, result, buf_size)
    return buffer_as_text(result)


def control_command_by_handle(hwnd: wintypes.HWND, ctrl: wintypes.HWND, command: str, extra: str,
                              buf_size: int = DEFAULT_CONTROL_COMMAND_BY_HANDLE_BUFFER_SIZE, *, _result=None) -> str:
    result = new_buffer(buf_size) if _result is None else _result
    API.AU3_ControlCommandByHandle(hwnd, ctrl, command, extra, result, buf_size)
    return buffer_as_text(result)


def control_list_view(title: str, text: str, control: str, command: str, extra1: str, extra2: str,
                      buf_size: int = DEFAULT_CONTROL_LIST_VIEW_BUFFER_SIZE, *, _result=None) -> str:
    result = new_buffer(buf_size) if _result is None else _result
    API.AU3_ControlListView(title, text, control, command, extra1, extra2, result, buf_size)
    return buffer_as_text(result)


def control_list_view_by_handle(hwnd: wintypes.HWND, ctrl: wintypes.HWND, command: str, extra1: str, extra2: str,
                                buf_size: int = DEFAULT_CONTROL_LIST_VIEW_BY_HANDLE_BUFFER_SIZE, *,
                                _result=None) -> str:
    result = new_buffer(buf_size) if _result is None else _result
    API.AU3_ControlListViewByHandle(hwnd, ctrl, command, extra1, extra2, result, buf_size)
    return buffer_as_text(result)
...
def control_get_focus(title: str, text: str, buf_size: int = DEFAULT_CONTROL_GET_FOCUS_BUFFER_SIZE, *,
                      _control_with_focus=None) -> str:
    control_with_focus = new_buffer(buf_size) if _control_with_focus is None else _control_with_focus
    API.AU3_ControlGetFocus(title, text, control_with_focus, buf_size)
    return buffer_as_text(control_with_focus)


def control_get_focus_by_handle(hwnd: wintypes.HWND, buf_size: int = DEFAULT_CONTROL_GET_FOCUS_BY_HANDLE_BUFFER_SIZE, *,
                                _control_with_focus=None) -> str:
    control_with_focus = new_buffer(buf_size) if _control_with_focus is None else _control_with_focus
    API.AU3_ControlGetFocusByHandle(hwnd, control_with_focus, buf_size)
    return buffer_as_text(control_with_focus)
...
def control_get_handle_as_text(title: str, text: str = '', control: str = '',
                               buf_size: int = DEFAULT_CONTROL_GET_HANDLE_AS_TEXT_BUFFER_SIZE, *,
                               _ret_text=None) -> str:
    ret_text = new_buffer(buf_size) if _ret_text is None else _ret_text
    API.AU3_ControlGetHandleAsText(title, text, control, ret_text, buf_size)
    return buffer_as_text(ret_text)


def control_get_pos(title: str, text: str, control: str, *, _rect=None) -> Tuple[int, Rect]:
    rect = wintypes.RECT() if _rect is None else _rect
    result = API.AU3_ControlGetPos(title, text, control, rect)
    return result, build_rect(rect)


def control_get_pos_by_handle(hwnd: wintypes.HWND, ctrl: wintypes.HWND, *, _rect=None) -> Tuple[int, Rect]:
    rect = wintypes.RECT() if _rect is None else _rect
    result = API.AU3_ControlGetPosByHandle(hwnd, ctrl, rect)
    return result, build_rect(rect)


def control_get_text(title: str, text: str, control: str, buf_size: int = DEFAULT_CONTROL_GET_TEXT_BUFFER_SIZE, *,
                     _control_text=None) -> str:
    control_text = new_buffer(buf_size) if _control_text is None else _control_text
    API.AU3_ControlGetText(title, text, control, control_text, buf_size)
    return buffer_as_text(control_text)


def control_get_text_by_handle(hwnd: wintypes.HWND, ctrl: wintypes.HWND,
                               buf_size: int = DEFAULT_CONTROL_GET_TEXT_BY_HANDLE_BUFFER_SIZE, *,
                               _control_text=None) -> str:
    control_text = new_buffer(buf_size) if _control_text is None else _control_text
    API.AU3_ControlGetTextByHandle(hwnd, ctrl, control_text, buf_size)
    return buffer_as_text(control_text)
...
def control_tree_view(title: str, text: str, control: str, command: str, extra1: str, extra2: str,
                      buf_size: int = DEFAULT_CONTROL_TREE_VIEW_BUFFER_SIZE, *, _result=None) -> str:
    result = new_buffer(buf_size) if _result is None else _result
    API.AU3_ControlTreeView(title, text, control, command, extra1, extra2, result, buf_size)
    return buffer_as_text(result)


def control_tree_view_by_handle(hwnd: wintypes.HWND, ctrl: wintypes.HWND, command: str, extra1: str, extra2: str,
                                buf_size: int = DEFAULT_CONTROL_TREE_VIEW_BY_HANDLE_BUFFER_SIZE, *,
                                _result=None) -> str:
    result = new_buffer(buf_size) if _result is None else _result
    API.AU3_ControlTreeViewByHandle(hwnd, ctrl, command, extra1, extra2, result, buf_size)
    return buffer_as_text(result)


def drive_map_add(device: str, share: str, flags: int, user: str = '', pwd: str = '',
                  buf_size: int = DEFAULT_DRIVE_MAP_ADD_BUFFER_SIZE, *, _result=None) -> str:
    result = new_buffer(buf_size) if _result is None else _result
    API.AU3_DriveMapAdd(device, share, flags, user, pwd, result, buf_size)
    return buffer_as_text(result)
...
def drive_map_get(device: str, buf_size: int = DEFAULT_DRIVE_MAP_GET_BUFFER_SIZE, *, _mapping=None) -> str:
    mapping = new_buffer(buf_size) if _mapping is None else _mapping
    API.AU3_DriveMapGet(device, mapping, buf_size)
    return buffer_as_text(mapping)
...
def mouse_get_pos(*, _point=None) -> Point:
    point = wintypes.POINT() if _point is None else _point
    API.AU3_MouseGetPos(point)
    return build_point(point)
...
def mouse_up(button: str = 'LEFT') -> None:
    API.AU3_MouseUp(button)


def mouse_wheel(direction: str, clicks: int) -> None:
    API.AU3_MouseWheel(direction, clicks)
...
def pixel_checksum(rect: Rect, step: int = 1) -> int:
    rect_obj = rect.to_wintypes()
    rect = ctypes.pointer(rect_obj)
...
def pixel_search(rect: Rect, col: int, var: int = 0, step: int = 1, *, _point_result=None) -> Point:
    rect_obj = rect.to_wintypes()
    rect = ctypes.pointer(rect_obj)
    point_result = wintypes.POINT() if _point_result is None else _point_result
    API.AU3_PixelSearch(rect, col, var, step, point_result)
    return build_point(point_result)
...
def sleep(milliseconds: int) -> None:
    API.AU3_Sleep(milliseconds)


def statusbar_get_text(title: str, text: str = '', part: int = 1,
                       buf_size: int = DEFAULT_STATUSBAR_GET_TEXT_BUFFER_SIZE, *, _status_text=None) -> Tuple[int, str]:
    status_text = new_buffer(buf_size) if _status_text is None else _status_text
    result = API.AU3_StatusbarGetText(title, text, part, status_text, buf_size)
    return result, buffer_as_text(status_text)


def statusbar_get_text_by_handle(hwnd: wintypes.HWND, part: int = 1,
                                 buf_size: int = DEFAULT_STATUSBAR_GET_TEXT_BY_HANDLE_BUFFER_SIZE, *,
                                 _status_text=None) -> Tuple[int, str]:
    status_text = new_buffer(buf_size) if _status_text is None else _status_text
    result = API.AU3_StatusbarGetTextByHandle(hwnd, part, status_text, buf_size)
    return result, buffer_as_text(status_text)


def tool_tip(tip: str, x: int = AU3_INTDEFAULT, y: int = AU3_INTDEFAULT) -> None:
    API.AU3_ToolTip(tip, x, y)
...
def win_get_caret_pos(*, _point=None) -> Tuple[int, Point]:
    point = wintypes.POINT() if _point is None else _point
    result = API.AU3_WinGetCaretPos(point)
    return result, build_point(point)


def win_get_class_list(title: str, text: str = '', buf_size: int = DEFAULT_WIN_GET_CLASS_LIST_BUFFER_SIZE, *,
                       _ret_text=None) -> str:
    ret_text = new_buffer(buf_size) if _ret_text is None else _ret_text
    API.AU3_WinGetClassList(title, text, ret_text, buf_size)
    return buffer_as_text(ret_text)


def win_get_class_list_by_handle(hwnd: wintypes.HWND, buf_size: int = DEFAULT_WIN_GET_CLASS_LIST_BY_HANDLE_BUFFER_SIZE,
                                 *, _ret_text=None) -> str:
    ret_text = new_buffer(buf_size) if _ret_text is None else _ret_text
    API.AU3_WinGetClassListByHandle(hwnd, ret_text, buf_size)
    return buffer_as_text(ret_text)


def win_get_client_size(title: str, text: str = '', *, _rect=None) -> Tuple[int, Rect]:
    rect = wintypes.RECT() if _rect is None else _rect
    result = API.AU3_WinGetClientSize(title, text, rect)
    return result, build_rect(rect)


def win_get_client_size_by_handle(hwnd: wintypes.HWND, *, _rect=None) -> Tuple[int, Rect]:
    rect = wintypes.RECT() if _rect is None else _rect
    result = API.AU3_WinGetClientSizeByHandle(hwnd, rect)
    return result, build_rect(rect)
...
def win_get_handle_as_text(title: str, text: str = '', buf_size: int = DEFAULT_WIN_GET_HANDLE_AS_TEXT_BUFFER_SIZE, *,
                           _ret_text=None) -> str:
    ret_text = new_buffer(buf_size) if _ret_text is None else _ret_text
    API.AU3_WinGetHandleAsText(title, text, ret_text, buf_size)
    return buffer_as_text(ret_text)


def win_get_pos(title: str, text: str = '', *, _rect=None) -> Tuple[int, Rect]:
    rect = wintypes.RECT() if _rect is None else _rect
    result = API.AU3_WinGetPos(title, text, rect)
    return result, build_rect(rect)


def win_get_pos_by_handle(hwnd: wintypes.HWND, *, _rect=None) -> Tuple[int, Rect]:
    rect = wintypes.RECT() if _rect is None else _rect
    result = API.AU3_WinGetPosByHandle(hwnd, rect)
    return result, build_rect(rect)
...
def win_get_text(title: str, text: str = '', buf_size: int = DEFAULT_WIN_GET_TEXT_BUFFER_SIZE, *,
                 _ret_text=None) -> str:
    ret_text = new_buffer(buf_size) if _ret_text is None else _ret_text
    API.AU3_WinGetText(title, text, ret_text, buf_size)
    return buffer_as_text(ret_text)


def win_get_text_by_handle(hwnd: wintypes.HWND, buf_size: int = DEFAULT_WIN_GET_TEXT_BY_HANDLE_BUFFER_SIZE, *,
                           _ret_text=None) -> str:
    ret_text = new_buffer(buf_size) if _ret_text is None else _ret_text
    API.AU3_WinGetTextByHandle(hwnd, ret_text, buf_size)
    return buffer_as_text(ret_text)


def win_get_title(title: str, text: str = '', buf_size: int = DEFAULT_WIN_GET_TITLE_BUFFER_SIZE, *,
                  _ret_text=None) -> str:
    ret_text = new_buffer(buf_size) if _ret_text is None else _ret_text
    API.AU3_WinGetTitle(title, text, ret_text, buf_size)
    return buffer_as_text(ret_text)


def win_get_title_by_handle(hwnd: wintypes.HWND, buf_size: int = DEFAULT_WIN_GET_TITLE_BY_HANDLE_BUFFER_SIZE, *,
                            _ret_text=None) -> str:
    ret_text = new_buffer(buf_size) if _ret_text is None else _ret_text
    API.AU3_WinGetTitleByHandle(hwnd, ret_text, buf_size)
    return buffer_as_text(ret_text)
...
def win_minimize_all() -> None:
    API.AU3_WinMinimizeAll()


def win_minimize_all_undo() -> None:
    API.AU3_WinMinimizeAllUndo()
